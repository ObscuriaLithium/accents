...
public enum AccentsCreativeTabs {
    ...
    interface Icons {
        ...
        static ItemStack general() {
            var stack = AccentsItems.CAKEMAN_PLUSHIE.get().getDefaultInstance();
            return DyedItemColor.applyDyes(stack, Contents.RED);
        }
    }
    ...
    interface Contents {
        ...
        static void general(CreativeModeTab.ItemDisplayParameters params, CreativeModeTab.Output output) {
            ...
            output.accept(AccentsItems.SEWING_STATION.get());
            output.accept(AccentsItems.SEWING_KIT.get());
            output.accept(AccentsItems.KASA_HAT.get());
        ...
        private static void acceptDyed(
                CreativeModeTab.Output output, Deferred<VanityItem> item,
                List<DyeItem> primary, List<DyeItem> secondary) {
            output.accept(item.get());
            output.accept(DyedItemColor.applyDyes(item.get().getDefaultInstance(), primary));
            output.accept(DyedItemColor.applyDyes(item.get().getDefaultInstance(), secondary));
        }
    }
